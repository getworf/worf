...
class Invitation(Base):
    ...
    def get_by_email(cls, session, tenant, email):
        return (
            session.query(cls)
            .join(User, User.id == cls.inviting_user_id)
            .filter(User.tenant == tenant, cls.email == email)
            .one_or_none()
        )
    ...
    def get_by_ext_id(cls, session, tenant, ext_id):
        return (
            session.query(cls)
            .join(User, User.id == cls.inviting_user_id)
            .filter(User.tenant == tenant, cls.ext_id == ext_id)
            .one_or_none()
        )

    @classmethod
    def get_by_token(cls, session, tenant, token):
        return (
            session.query(cls)
            .join(User, User.id == cls.inviting_user_id)
            .filter(User.tenant == tenant, cls.token == token)
            .one_or_none()
        )

    @classmethod
    def filter_by_type(cls, session, tenant, invitation_type):
        return (
            session.query(cls)
            .join(User, User.id == cls.inviting_user_id)
            .filter(User.tenant == tenant, cls.invitation_type == invitation_type)
        )
