...
AuthError = enum.Enum(
    "AuthError",
    ["NoToken", "TokenNotFound", "SuperuserRequire", "InvalidToken", "TokenExpired"],
)
...
def authorized(
    ...
    def rd(reason):
        return redirect(url_for(redirect_to) + f"?reason={reason.name}")
            ...
            with settings.session() as session:
                try:
                    ...
                    if anon_ok:
                        return process_anonymously()
                    if redirect_to:
                        return rd(AuthError.TokenNotFound)
                    # redirect to login
                    return invalid_response

                if not access_token.valid:
                    if redirect_to:
                        return rd(AuthError.InvalidToken)
                    return invalid_response

                if superuser and not user.superuser:
                    if redirect_to:
                        return rd(AuthError.SuperuserRequired)
                    return (
                ...
                if (
                    ...
                    if anon_ok:
                        return process_anonymously()
                    if redirect_to:
                        return rd(AuthError.TokenExpired)
                    return (
                        {
