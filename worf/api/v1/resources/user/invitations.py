...
class Invitations(Resource):
    ...
    def get(self, invitation_id=None):
        with settings.session() as session:
            if invitation_id is not None:
                invitation = Invitation.get_by_ext_id(
                    session, request.tenant, invitation_id
                )
    ...
    def post(self):
        ...
        with settings.session() as session:
            existing_invitation = Invitation.get_by_email(
                session, request.tenant, invitation.email
            )
