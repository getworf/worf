...
def get_provider(name, session):
    providers = settings.providers.get("signup.{}".format(name))
    if not providers:
        return None
    return providers[0](session)
...
def create_signup_request(session, data):
    signup_request = SignupRequestModel.get_or_create(
        session, request.tenant, settings.salted_hash(data["email"])
    )
...
def finalize(session, tenant, resource, data, generate_token=True):
    ...
    if User.get_by_email(session, tenant, data["email"]):
    ...
    if data.get("invitation"):
        invitation = Invitation.get_by_token(session, tenant, data["invitation"])
        if invitation:
...
class ConfirmSignup(Resource):
    def get(self):
        ...
        with settings.session() as session:
            ...
            token.used = True
            return finalize(session, request.tenant, self, data)


class SignupRequests(Resource):
    def _handle_request(self, request_id, confirm):
        with settings.session() as session:
            signup_request = SignupRequestModel.get_by_ext_id(
                session, request.tenant, request_id
            )
            if signup_request is None:
                return {"message": self.t("not-found")}, 404
            data = signup_request.encrypted_data
            session.delete(signup_request)
            if confirm:
                if data.get("email_verified"):
                    return finalize(
                        session, request.tenant, self, data, generate_token=False
                    )
    ...
    def get(self):
        with settings.session() as session:
            signup_requests = (
                ...
                .filter(SignupRequestModel.tenant == request.tenant)
                .order_by(desc(SignupRequestModel.created_at))
                .all()
            )
            return {
                "requests": [
                    signup_request.export() for signup_request in signup_requests
                ]
            }, 200
...
class Signup(Resource):
    def post(self, provider_name):
        ...
        with settings.session() as session:
            ...
            if User.get_by_email(session, request.tenant, data["email"]):
                return {"message": self.t("signup.email-already-taken")}, 400

            if data.get("invitation"):
                invitation = Invitation.get_by_token(
                    session, request.tenant, data["invitation"]
                )
                if invitation:
            ...
            if approve:
                return create_signup_request(session, data)
