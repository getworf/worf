...
def generate_access_token(user, provider, session, trusted=False, extra_data=None):
    ...
    access_token.description = "Login from IP {} with user agent {}".format(
        request.anon_ip if hasattr(request, "anon_ip") else request.originating_ip,
...
def generate_and_return_token(user, provider, session, trusted=False, extra_data=None):
    access_token = generate_access_token(
        user, provider, session, trusted=trusted, extra_data=extra_data
    )
...
class Login(Resource):
    ...
    def post(self, provider_name):
        ...
        * Let is log in the user
        ...
        with settings.session() as session:
            providers = settings.providers.get("login.{}".format(provider_name))
            if not providers:
